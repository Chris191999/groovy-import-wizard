...
const TradeList = ({ trades, onEdit, onDelete, compact = false }: TradeListProps) => {
  ...
  const getFirstTradeImage = (trade: Trade) => {
    ...
    const firstImageName = trade.image_files[0];
    const imageData = localStorage.getItem(firstImageName);
    console.log(`Getting first image for trade ${trade.id} using key ${firstImageName}:`, imageData ? 'Found' : 'Not found');
    return imageData;
  };
  ...
  function TradeTableRow({
    ...
    getPnLColor: (pnl: number) => string;
                ...
                onClick={() => onView(trade)}
                      ...
                      <TableCell>
                        {trade.exit ? formatCurrency(trade.exit) : '-'}
                      </TableCell>
